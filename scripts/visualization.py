...
def add_value_labels(ax, spacing=5):
    ...
    # for p in ax.patches:
    #     ax.annotate('{:.0f}'.format(p.get_height()), (p.get_x()+0.4, p.get_height()-p.get_height()+10),
    #                 ha='center', va='bottom',
    #                 color= 'white')
...
def plot_bargraph(df:pd.core.frame.DataFrame,
    ...
    if conf_interval:
        ax = sns.barplot(x = x_axis_column, y = y_axis_column, hue=hue_column_name, data=df, order=order)
    else:
        ax = sns.barplot(x = x_axis_column, y = y_axis_column, hue=hue_column_name, data=df, order=order, ci=None)
